...
/*
	Сортировка слиянием.
	Плюсы:
		- Средняя скорость работы алгоритма - O(NlogN)
		- Худшая скорость работы алгоритма - O(NlogN)
	Минусы:
		- Затраты памяти - O(N)
		- Лучшая скорость - O(NlogN)

	Данный алгоритм хорош в большинстве ситуаций, потому что 
	скорость его работы хорошо прогнозируема и не зависит от порядка следования элементов,
	что позволяет прогнозировать производительность программы(в данном случае, делать расчет бюджета кадра)
*/
template<typename T>
